...
def brushStep(app_token, user_id, step):
    ...
    """
    # 【注意】这里的 data_json 需要替换为你抓包时获取的真实 data_json 结构
    ...
    # 替换日期和步数（如果 data_json 是占位符，这里的正则可能会报错，测试时请注意）
    try:
        ...
        print("警告: 无法在 data_json 中正则匹配到日期或步数字段，请检查 data_json 格式。")
...
def main(app_token, user_id, _step_min, _step_max):
    step = str(random.randint(_step_min, _step_max))
    print("已设置为随机步数:" + step)
    
    if not app_token or not user_id:
        return "请正确填写 app_token 或 user_id"

    # 直接使用传入的 app_token 和 user_id 刷步
    brush, message = brushStep(app_token, user_id, step)
...
if __name__ == '__main__':
    ...
    # 刷步数
    res = main(APP_TOKEN, USER_ID, 12001, 15301)
    # 推送消息
    if PUSHPLUS_TOKEN:
